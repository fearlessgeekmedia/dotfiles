...
echo -e "${BLUE}Copying Netlify configuration...${NC}"
if [ -f "_redirects" ]; then
    cp _redirects "$EXPORT_DIR/"
    echo -e "${GREEN}✓ _redirects copied${NC}"
fi
if [ -f "_headers" ]; then
    cp _headers "$EXPORT_DIR/"
    echo -e "${GREEN}✓ _headers copied${NC}"
fi
