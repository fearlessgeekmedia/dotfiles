...
function blog_generate_rss() {
    ...
    if (file_exists($configFile)) {
        ...
        // Use configured site URL if available, otherwise fall back to current host
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            if (!function_exists('validate_csrf_token') || !validate_csrf_token()) {
                ...
                if (isset($_POST['action']) && $_POST['action'] === 'save_post') {
                    ...
                    if ($title && $slug) {
                        // Normalize featured image URL to use relative paths
                        $featured_image = $_POST['featured_image'] ?? '';
                        if (!empty($featured_image)) {
                            // If it's a full URL with localhost, convert to relative path
                            if (strpos($featured_image, 'http://localhost:8000/') === 0) {
                                $featured_image = substr($featured_image, 20); // Remove 'http://localhost:8000/'
                            } elseif (strpos($featured_image, 'https://localhost:8000/') === 0) {
                                $featured_image = substr($featured_image, 21); // Remove 'https://localhost:8000/'
                            }
