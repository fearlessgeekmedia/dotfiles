...
fcms_register_admin_section('blog', [
    ...
    'render_callback' => function() {
        ...
        #blog-code-editor:focus {
            outline: none;
            border-color: #3b82f6;
            box-shadow: 0 0 0 3px rgba(59, 130, 246, 0.1);
        }
        
        #blog-code-editor-container {
        ...
        if (isset($_GET['edit'])) {
            ...
            if (!$edit) {
                ...
                echo '<p class="text-sm text-gray-600 mb-2">Editing in HTML mode with Quill.js editor</p>';
                echo '<div class="mb-3 flex items-center space-x-2">';
                echo '<button type="button" id="toggleMode" class="px-3 py-1 text-sm bg-blue-500 text-white rounded hover:bg-blue-600 transition-colors">Switch to Code View</button>';
                echo '<span class="text-sm text-gray-600" id="modeIndicator">Rich Editor Mode</span>';
                echo '<span class="text-xs text-gray-500">(Ctrl+Shift+C to toggle)</span>';
                echo '</div>';
                echo '<div id="blog-rich-editor" class="quill-editor" style="height: 500px;"></div>';
                echo '<div id="blog-code-editor-container" class="hidden">';
                echo '<textarea id="blog-code-editor" class="w-full h-96 p-4 font-mono text-sm border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500" placeholder="Edit HTML code here..."></textarea>';
                echo '<div class="mt-2 text-sm text-gray-600">';
                echo '<p>💡 <strong>Code View Mode:</strong> Edit raw HTML code. Use this for precise formatting, custom HTML, or tables.</p>';
                echo '</div>';
                echo '</div>';
                ...
                // Quill.js Editor initialization with HTML editing mode
                echo '<script>
                document.addEventListener("DOMContentLoaded", function() {
                    ...
                    editor = new Quill("#blog-rich-editor", {
                        theme: "snow",
                        modules: {
                    ...
                    document.getElementById("toggleMode").addEventListener("click", function() {
                        var richEditor = document.getElementById("blog-rich-editor");
                        var codeEditorContainer = document.getElementById("blog-code-editor-container");
                        var codeEditor = document.getElementById("blog-code-editor");
                        var toggleButton = document.getElementById("toggleMode");
                        var modeIndicator = document.getElementById("modeIndicator");

                        if (richEditor.classList.contains("hidden")) {
                            // Switching from Code View to Rich Editor
                            var htmlContent = codeEditor.value;
                            ...
                            codeEditor.value = editor.root.innerHTML;
                        }
                    });
                    ...
                    document.getElementById("blog-post-form").addEventListener("submit", function(e) {
                        ...
                        if (richEditor.classList.contains("hidden")) {
                            ...
                            contentToSave = editor.root.innerHTML;
                        }
                        
                        document.getElementById("blog-editor-content").value = contentToSave;
                    });
                });
                </script>';
            }
            ...
            // Quill.js Editor initialization for new post with HTML editing mode
            echo '<script>
            document.addEventListener("DOMContentLoaded", function() {
                var editor;
