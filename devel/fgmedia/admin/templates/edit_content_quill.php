...
.quill-editor .ql-editor table tr:nth-child(even) {
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (typeof Quill !== 'undefined') {
        ...
        // Add custom table button and handler
        var tableButton = document.createElement('button');
        tableButton.innerHTML = '<svg viewBox="0 0 18 18"><rect class="ql-stroke" x="3" y="3" width="12" height="12"></rect><line class="ql-stroke" x1="9" y1="3" x2="9" y2="15"></line><line class="ql-stroke" x1="3" y1="9" x2="15" y2="9"></line></svg>';
        ...
        tableButton.addEventListener('click', function() {
            ...
            if (rows && cols && !isNaN(rows) && !isNaN(cols) && rows > 0 && cols > 0) {
                ...
                var tableHTML = '<table class="ql-table" style="border-collapse: collapse; width: 100%; margin: 1em 0;"><tbody>';
                for (var r = 0; r < rows; r++) {
                    tableHTML += '<tr>';
                    for (var c = 0; c < cols; c++) {
                        var cellTag = (r === 0) ? 'th' : 'td';
                        var cellStyle = (r === 0) ? 'font-weight: bold; background-color: #f3f4f6; padding: 8px; border: 1px solid #d1d5db;' : 'padding: 8px; border: 1px solid #d1d5db;';
        ...
        editor.clipboard.addMatcher('td, th', function(node, delta) {
            var Delta = Quill.import('delta');
            return new Delta().insert(node.outerHTML);
        });
        
        editor.clipboard.addMatcher('tbody, thead, tfoot', function(node, delta) {
            var Delta = Quill.import('delta');
            return new Delta().insert(node.outerHTML);
        });
        
        // Handle paste event to detect and insert table HTML
        editor.root.addEventListener('paste', function(e) {
            e.preventDefault();
            var clipboardData = e.clipboardData || window.clipboardData;
            var pastedHTML = clipboardData.getData('text/html');
            var pastedText = clipboardData.getData('text/plain');
            var pastedData = pastedHTML || pastedText;
            
            // Check if pasted content contains table HTML (either as HTML or as text)
            if (pastedData && /<table[\s\S]*?<\/table>/i.test(pastedData)) {
                // Extract all table HTML (handle multiple tables)
                var tableMatches = pastedData.match(/<table[\s\S]*?<\/table>/gi);
                if (tableMatches && tableMatches.length > 0) {
                    ...
                    tableMatches.forEach(function(tableHTML) {
                        // Clean up the table HTML - normalize styling
                        tableHTML = tableHTML.replace(/style="[^"]*"/gi, function(match) {
                            // Keep essential styles but normalize
                            if (match.includes('border') || match.includes('padding') || match.includes('width') || match.includes('collapse')) {
                                return match;
                            }
                            return '';
                        });
                    
                        // Ensure table has basic styling
                        if (!tableHTML.match(/<table[^>]*style=/i)) {
                            tableHTML = tableHTML.replace(/<table/i, '<table style="border-collapse: collapse; width: 100%; margin: 1em 0;"');
                        }
                        
                        // Ensure cells have basic padding
                        tableHTML = tableHTML.replace(/<td([^>]*)>/gi, function(match, attrs) {
                            if (!attrs.includes('style')) {
                                return '<td style="padding: 8px; border: 1px solid #d1d5db;">';
                            } else if (!attrs.includes('padding')) {
                                return match.replace(/style="([^"]*)"/i, 'style="$1; padding: 8px; border: 1px solid #d1d5db;"');
                            }
                            return match;
                        });
                        
                        tableHTML = tableHTML.replace(/<th([^>]*)>/gi, function(match, attrs) {
                            if (!attrs.includes('style')) {
                                return '<th style="padding: 8px; border: 1px solid #d1d5db; font-weight: bold; background-color: #f3f4f6;">';
                            } else if (!attrs.includes('padding')) {
                                return match.replace(/style="([^"]*)"/i, 'style="$1; padding: 8px; border: 1px solid #d1d5db; font-weight: bold; background-color: #f3f4f6;"');
                            }
                            return match;
                        });
                        
                        // Insert the table
                        editor.clipboard.dangerouslyPasteHTML(insertIndex, tableHTML);
                        insertIndex = editor.getLength();
                    });
                    
                    // Set cursor after the last table
                    editor.setSelection(insertIndex, 'api');
                    return;
                }
            }
            
            // For non-table content, use default paste behavior
            var selection = editor.getSelection(true);
            if (selection) {
                var delta = editor.clipboard.convert(pastedData);
                editor.updateContents(delta, 'user');
                editor.setSelection(selection.index + delta.length(), 'api');
            }
        });
        ...
        function loadContentIntoEditor() {
            ...
            if (initialContent && initialContent.trim()) {
                ...
                // Note: Tables are now supported, so we don't treat them as complex HTML
                const hasComplexHTML = /<div[^>]*style=|<div[^>]*class=|grid|parallax|shortcode|\[.*?\]|<form|<input|<textarea|<select|<button|<thead|<tfoot|<colgroup|<col|<caption|<fieldset|<legend|<label|<optgroup|<option|<datalist|<output|<progress|<meter|<details|<summary|<dialog|<menu|<menuitem|<slot|<template|<svg|<canvas|<video|<audio|<iframe|<embed|<object|<param|<source|<track|<map|<area|<picture|<figure|<figcaption|<nav|<header|<footer|<main|<section|<article|<aside|<hgroup|<address|<blockquote|<pre|<kbd|<samp|<var|<time|<mark|<ruby|<rt|<rp|<bdi|<bdo|<span[^>]*style=|<span[^>]*class=/i.test(initialContent);
